 ...
 * PHP Version 5
 *
 * @file     CASSHIB/PGTStorage/Db.php
...
define('CASSHIB_PGT_STORAGE_DB_DEFAULT_TABLE', 'CASSHIB_pgts');

/**
 * Basic class for PGT database storage
 * The CASSHIB_PGTStorage_Db class is a class for PGT database storage.
 *
 * @class    CASSHIB_PGTStorage_Db
 * @category Authentication
 * @package  PhpCASSHIB
 * @author   Daniel Frett <daniel.frett@gmail.com>
 * @license  http://www.apache.org/licenses/LICENSE-2.0  Apache License 2.0
...
class CASSHIB_PGTStorage_Db extends CASSHIB_PGTStorage_AbstractStorage
{
    /**
     * @addtogroup internalCASSHIB_PGTStorageDb
     * @{
     */
     ...
     *
     * @param CASSHIB_Client $CASSHIB_parent     the CASSHIB_Client instance that creates
     * the object.
    ...
    public function __construct(
        $CASSHIB_parent, $dsn_or_pdo, $username='', $password='', $table='',
        $driver_options=null
    ) {
        ...
        // set default values
        if ( empty($table) ) {
            $table = CASSHIB_PGT_STORAGE_DB_DEFAULT_TABLE;
        }
    ...
    public function init()
    {
        ...
        if (!($this->_pdo instanceof PDO)) {
            try {
                ...
                phpCASSHIB::error('Database connection error: ' . $e->getMessage());
            }
        }
    ...
    public function createTable()
    {
        ...
        try {
            ...
            // attempt rolling back the transaction before throwing a phpCASSHIB error
            try {
                $pdo->rollBack();
            }
            catch(PDOException $e) {
            }
            phpCASSHIB::error('error creating PGT storage table: ' . $e->getMessage());
        }
    ...
    public function write($pgt, $pgt_iou)
    {
        ...
        try {
            ...
            phpCASSHIB::error('error writing PGT to database: ' . $e->getMessage());
        }
    ...
    public function read($pgt_iou)
    {
        ...
        try {
            ...
            phpCASSHIB::trace('error reading PGT from database: ' . $e->getMessage());
        }
